...
缓存迁移说明：
    - 手动缓存（_cache/_get_from_cache/_set_to_cache）已移除定义 ✅
    - 核心方法缓存调用已注释（get_stock_list, get_stock_info, get_kline, get_realtime_quote）✅
    - instruments 相关方法已改用 storage_manager ✅
    - 统一使用 storage_manager 的 EXCHANGE 分类存储
    - 优先使用 @api_call_cache 装饰器和 smart_loader
"""
import asyncio
import time

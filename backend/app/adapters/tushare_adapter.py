        ...
        try:
            ...
            for _, row in df.iterrows():
                tick_data = {
                    ...
                    "成交价": float(row["close"]),
                    "手数": int(row["vol"]),
                    "买卖盘性质": "B" if row.get("buy_sm", 0) > row.get("sell_sm", 0) else "S"
                }
        ...
        try:
            ...
            for _, row in df.iterrows():
                result.append({
                    ...
                    "buy_sm_amount": float(row["buy_sm_amount"]) if "buy_sm_amount" in row else None,
                    "sell_sm_amount": float(row["sell_sm_amount"]) if "sell_sm_amount" in row else None,
                    "buy_md_amount": float(row["buy_md_amount"]) if "buy_md_amount" in row else None,
                    "sell_md_amount": float(row["sell_md_amount"]) if "sell_md_amount" in row else None,
                    "buy_lg_amount": float(row["buy_lg_amount"]) if "buy_lg_amount" in row else None,
                    "sell_lg_amount": float(row["sell_lg_amount"]) if "sell_lg_amount" in row else None,
                    "buy_elg_amount": float(row["buy_elg_amount"]) if "buy_elg_amount" in row else None,
                    "sell_elg_amount": float(row["sell_elg_amount"]) if "sell_elg_amount" in row else None,
                    "net_mf_amount": float(row["net_mf_amount"]) if "net_mf_amount" in row else None
                })

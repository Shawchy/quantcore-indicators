...
class TushareAPICache:
    ...
    def __init__(self, default_ttl: int = 300, max_size: int = 1000):
        ...
        logger.info(f"Tushare 缓存管理器初始化完成 (TTL={default_ttl}s, max_size={max_size})")
...
class TushareAPIStats:
    ...
    _instance: Optional['TushareAPIStats'] = None
    
    def __new__(cls) -> 'TushareAPIStats':
    ...
    def __init__(self):
        ...
        logger.info("Tushare API 统计管理器初始化完成")
...
# 全局单例
api_cache = TushareAPICache(default_ttl=300, max_size=1000)
api_stats = TushareAPIStats()


def get_api_cache() -> TushareAPICache:
    """获取缓存管理器单例"""
    return api_cache


def get_api_stats() -> TushareAPIStats:
    """获取统计管理器单例"""
    return api_stats

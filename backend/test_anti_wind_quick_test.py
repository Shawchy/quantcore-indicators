...
async def test_basic_functionality():
    ...
    # 1. 测试 Facade 初始化（使用 BASIC_CONFIG 减少延迟）
    ...
    # 3. 测试策略启用/禁用
    facade.disable_strategy('RateLimitStrategy')  # 禁用限流以加快测试
    assert len(facade.get_enabled_strategies()) < len(facade.strategies)
    print(f"✅ 禁用策略成功")
    
    facade.enable_strategy('RateLimitStrategy')
    print(f"✅ 启用策略成功")
...
if __name__ == "__main__":
    # 设置日志
    logger.remove()
    logger.add(sys.stdout, level="INFO", format="HH:mm:ss | {message}")
